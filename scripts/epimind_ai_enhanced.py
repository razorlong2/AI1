...
- Predicție automată IAAM cu algoritmi îmbunătățiți
    ...
    def predict_iaam_risk(self, data: PatientData) -> Dict:
        ...
        if hours < 48:
            return {
                ...
                "recommendations": ["Monitorizare standard", "Reevaluare la 48h"],
                "is_iaam": False,
        ...
        if 48 <= hours < 72:
            time_score = 8
            details.append(f"Spitalizare {hours}h - risc timpuriu (+{time_score})")
        elif hours < 168:  # < 1 săptămână
            time_score = 15
            details.append(f"Spitalizare {hours}h - risc moderat (+{time_score})")
        elif hours < 336:  # < 2 săptămâni
            time_score = 25
            details.append(f"Spitalizare {hours}h - risc înalt (+{time_score})")
        else:
            time_score = 35
            details.append(f"Spitalizare {hours}h - risc foarte înalt (+{time_score})")
            ...
            for pattern in patterns:
                ...
                if match:
                    try:
                        ...
                        if key == "ore_spitalizare" and "zile" in text_lower:
        ...
        if calc_button:
            if chat.can_calculate_risk():
                with st.spinner("🧮 Calculez riscul IAAM..."):
                    result = chat.calculate_and_display_risk()
                    if result:
